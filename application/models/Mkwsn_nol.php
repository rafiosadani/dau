...
class Mkwsn_nol extends CI_Model {
    ...
    public function getKawasanNol($kodej,$tgl1,$tgl2) {
        ...
        $day = $hari+1;
        $tgl2 = $tahun."-".$bln."-".$day;
        $query = $this->db->query("SELECT z.kwsn,z.Bulan,z.tahun,z.kodej,SUM(jumlah) as jumlah,SUM(infaq) as infaq ,count(DISTINCT z.noid) as noid,x.kwsn,x.kwsn_lm,rk,ins_pk from (select Bulan,tahun,kodej,a.noid_new AS noid,SUM(infaq) as infaq, a.kwsn FROM tagihandonatur a where Bulan=".$bulan." and kodej='".$kodej."' group by a.noid_new) z left join (SELECT noid,SUM(jumlah) as jumlah FROM report_tagih WHERE ((tanggal>= '".$tgl1."' and tanggal<= '".$tgl2."') OR tanggal like '".$tgl1."%' ) and (jenis='1' OR jenis='2' OR jenis='3' OR jenis='4' OR jenis='5' OR start!='') and kodej='".$kodej."' group by noid) y on y.noid= z.noid left join kawasan x on x.kwsn=z.kwsn WHERE jumlah IS NULL group by z.kwsn order by z.kwsn");
        return $query->result();
		}
		
		public function getTarget($bulan,$tahun,$kodej,$kwsn){
				if($kwsn == ''){
					$query = $this->db->query("select sum(infaq) as t_dns, count(noid) as t_dnt from tagihandonatur where Bulan = ".$bulan." and tahun = ".$tahun." and kodej = ".$kodej." group by kwsn ");
        }else{
					$query = $this->db->query("select sum(infaq) as t_dns, count(noid) as t_dnt from tagihandonatur where Bulan = ".$bulan." and tahun = ".$tahun." and kodej = ".$kodej." and kwsn = ".$kwsn." group by kwsn ");
				}
			  return $query->result();
		}
