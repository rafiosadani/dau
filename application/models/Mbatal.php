...
public function search($kodej,$kwsn){
    ...
    $query = $this->db->query("select a.report_id,a.noid,b.nama,b.kwsn,b.almktr,a.tanggal,a.validasi,c.NM_PROGRAM,jumlah,a.prog from report_tagih a left join donaturbaru b on a.noid=b.noid left join program c on a.prog=c.PROG
    where noslip!=0 and id=0 and batal IS NULL  and a.kodej ='$kodej' and kwsn='$kwsn'  and a.prog!='' 
    and substring(a.tanggal,1,10) >= '$tgl1' and substring(a.tanggal,1,10) <='$tgl2' and a.jumlah <> '0' order by b.nama asc");
    return $query->result();
    return $query->result();
}
