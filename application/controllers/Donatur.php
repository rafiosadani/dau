...
class Donatur extends CI_Controller {
    ...
    public function donatur()
    {
        if ($this->session->userdata('login') == TRUE) {
            if ($this->session->userdata('superadmin') == TRUE) {
				...
				$proza = "select name,kodej from sec_users where kodej > 0 and kodej !='' order by idcabang asc";
				...
				$proza = "select name,kodej from sec_users where kodej > 0 and kodej !='' and idcabang = ".$this->session->userdata('idcab')." order by idcabang asc";
				...
				$proza = "select name,kodej from sec_users where kodej > 0 and kodej !='' and group_id = ".$this->session->userdata('idgrup')." order by idcabang asc";
			} else {
				...
				$proza = "select name,kodej from sec_users where kodej > 0 and kodej !='' and kodej = ".$this->session->userdata('ses_kodej')." order by idcabang asc";
			}
            $program = "SELECT PROG, NM_PROGRAM FROM program WHERE status = 'A'";
			$data['petugas']=$this->M_donatur->query($kodej);
			$data['proza']=$this->M_donatur->query($proza);
    ...
    public function donasi(){
      if ($this->session->userdata('login') == TRUE) {
        $data['donasi']    = $this->M_donatur->query("select noid from donaturbaru");
        $data['program']    = $this->M_donatur->query("select * from program");
        $data['dtprozis']    = $this->M_donatur->query("select kodej from sec_users where lapangan ='A' group by kodej");
    ...
    public function addDonatur($value='')
    {
        if ($this->session->userdata('admin') == TRUE) {
            if ($this->input->post('nama') == NULL || $this->input->post('nama') == "") {
                ...
                $data['jungut'] = $this->M_donatur->query("select kodej,name from sec_users where kodej != ''");
    ...
    public function getDnsi(){
      $this->db->select('donaturbaru.nama,donaturbaru.kwsn,donaturbaru.alamat,donaturbaru.jupen as prozis,kawasan.kodejgt as kdprozis,kawasan.ins_pk as nm_kawasan, donatur_item.besar as nominal ');
      $this->db->from('donaturbaru');
      $this->db->join('kawasan','kawasan.kwsn = donaturbaru.kwsn','left');
      $this->db->join('donatur_item','donatur_item.noid = donaturbaru.autoid','left');
      $this->db->where('donaturbaru.noid', $this->input->get('iddonatur'));
        ...
        if ($this->session->userdata('login') == TRUE) {
            if ($this->session->userdata('keyword') == NULL) {
                ...
                if ($this->input->post()) {
					...
					$this->session->userdata('keyword')['proza'] != $this->input->post('proza')) {
                    $array = array(
                        'keyword' => array(
                            'kwsn' => $this->input->post('kwsn'),
                            'status' => $this->input->post('status'),
    ...
    public function addKeu_j(){
      if ($this->input->post('program') && $this->input->post('nominal') && $this->input->post('kodeprozis')){
        $this->M_donatur->insertKeu_j();
        $this->M_donatur->insertReport_tagih();
        $data['noid']= $this->input->post('iddonatur');
        $data['nama']= $this->input->post('nama');
        $data['alamat']= $this->input->post('alamat');
        $data['instansi']= $this->input->post('instansi');
        $data['kodekawasan']= $this->input->post('kodekawasan');
        $data['kodeprozis']= $this->input->post('kodeprozis');
        $data['nominal']= $this->input->post('nominal');
        $this->load->view('cetakdonasi', $data);
      }else {
        redirect(base_url('dashboard'));
      }
    }
