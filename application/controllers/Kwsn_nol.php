...
class Kwsn_nol extends CI_Controller {
    ...
    public function dataExcel(){
      if ($this->session->userdata('admin') == TRUE) {
        ...
        ->mergeCells('A1:O1')->getStyle('A1')
        ->getAlignment()->setHorizontal('center');
        $sheet->setCellValue('A2','REKAP KAWASAN KURANG TARGET ')
        ->mergeCells('A2:O2')->getStyle('A2')
        ->getAlignment()->setHorizontal('center');
        $sheet->setCellValue('A3','KODE PROZIS : '.$jungut.'     PERIODE : '.$tgl1.' S/D '.$tgl2.'     TANGGAL CETAK : '.$tanggal_cetak)
        ->mergeCells('A3:O3')->getStyle('A3')
        ->getAlignment()->setHorizontal('center');
        $sheet->setCellValue('A4','')
        ->mergeCells('A4:O4')->getStyle('A4')
        ->getAlignment()->setHorizontal('center');
        ...
        $sheet->setCellValue('D5','Nama Kawasan')
          ...
          ->getAlignment()->setVertical('center')
          ->setHorizontal('left');
        $sheet->setCellValue('L5','Gagal Tagih')
          ->mergeCells('L5:N5')->getStyle('L5')
          ->getAlignment()->setVertical('center')
          ->setHorizontal('left');
        $sheet->setCellValue('O5','RK')
          ->mergeCells('O5:O6')->getStyle('O5')
          ->getAlignment()->setVertical('center')
          ->setHorizontal('left');
        ...
        foreach ($data['kwsnNol'] as $no => $tampil) {
          ...
          $sheet->setCellValue('D'.$row,$tampil->ins_pk)
            ...
            ->getAlignment()->setVertical('center')
						->setHorizontal('left');
					foreach ($this->mkwsn_nol->getTarget($tampil->Bulan,$tampil->tahun,$tampil->kodej,$tampil->kwsn) as $v) :
						$sheet->setCellValue('I'.$row,$v->t_dnt);
						$sheet->setCellValue('J'.$row,number_format($v->t_dns,0,'.',','))
							->mergeCells('J'.$row.':K'.$row)->getStyle('J'.$row)
							->getAlignment()->setHorizontal('right');	
					endforeach;
          $sheet->setCellValue('L'.$row,$tampil->noid);
          $sheet->setCellValue('M'.$row,number_format($tampil->infaq,0,'.',','))
            ->mergeCells('M'.$row.':N'.$row)->getStyle('M'.$row)
            ->getAlignment()->setHorizontal('right');
          $sheet->setCellValue('O'.$row,$tampil->rk);
        ...
        ->getAlignment()->setHorizontal('center');
				$tot = 0;
				foreach ($this->mkwsn_nol->getTarget($tampil->Bulan,$tampil->tahun,$tampil->kodej,'') as $v) {
					$tot = $v->t_dnt+$tot;
				}
        ...
        $sheet->setCellValue('L'.$row,number_format($tot,0,'.',','))->getStyle('L'.$row)
        ->getAlignment()->setHorizontal('right');
        ...
        $sheet->getStyle('A5:O'.$row)->applyFromArray($styleArray);
        ...
        $writer->save('php://output');$sheet->getStyle('A5:O'.$row)->applyFromArray($styleArray);
        $writer = new Xlsx($spreadsheet);
